...
PerspectiveCamera.prototype.move = function(dt, keysPressed) {
  if(this.isDragging){ this.frameCount = 1.0;
  ...
  if(keysPressed.W) { this.frameCount = 1.0;
    this.position.addScaled(this.speed * dt, this.ahead);
  }
  if(keysPressed.S) { this.frameCount = 1.0;
    this.position.addScaled(-this.speed * dt, this.ahead);
  }
  if(keysPressed.D) { this.frameCount = 1.0;
    this.position.addScaled(this.speed * dt, this.right);
  }
  if(keysPressed.A) { this.frameCount = 1.0;
    this.position.addScaled(-this.speed * dt, this.right);
  }
  if(keysPressed.E) { this.frameCount = 1.0;
    this.position.addScaled(this.speed * dt, PerspectiveCamera.worldUp);
  }
  if(keysPressed.Q) { this.frameCount = 1.0;
    this.position.addScaled(-this.speed * dt, PerspectiveCamera.worldUp);
  }
